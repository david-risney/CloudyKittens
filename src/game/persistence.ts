...
import type { StoragePort, ClockPort } from './ports.js';
...
function isValidState(value: unknown): value is GameState {
  ...
  if (!Array.isArray(s.cats)) return false;
  if (s.cats.length > HOME_CAPACITY) return false;
  if (typeof s.lastSeen !== 'number' || !Number.isFinite(s.lastSeen)) return false;
  ...
  if (typeof player.firstCatClaimed !== 'boolean') return false;

  const shop = s.shop as Record<string, unknown> | undefined;
  if (!shop || typeof shop !== 'object') return false;
  if (typeof shop.lastShopDay !== 'string') return false;
  if (!Array.isArray(shop.adoptableCats)) return false;
...
export function save(state: GameState, storage: StoragePort, clock: ClockPort): void {
  const toStore: GameState = { ...state, lastSeen: clock.now() };
  try {
 ...
 */
export function load(storage: StoragePort, clock: ClockPort): GameState {
  ...
  if (raw === null) return newGame(clock);
  ...
  if (!isValidState(parsed)) return newGame(clock);
