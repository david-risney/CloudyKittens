...
export type AdoptResult = {
  state: GameState;
  adopted: boolean;
  reason?: 'full' | 'no-funds';
};

export type BuyResult = { state: GameState; bought: boolean };
...
export function refreshIfNewDay(state: GameState, clock: ClockPort): GameState {
  if (!isNewDay(state.shop.lastShopDay, clock)) return state;
 ...
 */
export function adopt(state: GameState, catIndex: number, clock: ClockPort): AdoptResult {
...
export function buyItem(state: GameState, item: ItemId): BuyResult {
