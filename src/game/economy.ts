import type { GameState, Cat, ItemId } from './types.js';
...
function findCat(state: GameState, catId: string): Cat | undefined {
  return state.cats.find((c) => c.id === catId);
}

function moneyForTrust(trust: number): number {
  if (trust <= 0) return 0;
 ...
 * scaled by trust.
 */
export function feed(state: GameState, catId: string, item: ItemId): FeedResult {
  const noop: FeedResult = { state, consumed: false, moneyGained: 0, trustDelta: 0 };

  const current = findCat(state, catId);
  if (!current) return noop;
  if (!isFood(item)) return noop;
  if ((state.player.inventory[item] ?? 0) <= 0) return noop;
  if (current.hunger <= 0) return noop; // already full → cancel (FR-005)
...
export function useToy(state: GameState, catId: string, item: ItemId): GameState {
  const current = findCat(state, catId);
  if (!current) return state;
  if (!isToy(item)) return state;
...
export function pet(state: GameState, catId: string): GameState {
  const current = findCat(state, catId);
  if (!current) return state;
