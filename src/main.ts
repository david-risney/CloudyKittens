...
let state: GameState = load(storage, clock);
ensureStarterCat();
...
let selectedCatId: string | null = state.cats[0]?.id ?? null;

const canvas = document.getElementById('scene') as HTMLCanvasElement;
const ctx = canvas.getContext('2d')!;
const hudRoot = document.getElementById('ui') as HTMLElement;
...
function ensureStarterCat(): void {
