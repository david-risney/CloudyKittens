...
function clockFor(today, now = 1_000_000) {
...
describe('refreshIfNewDay', () => {
    it('keeps the same roster on the same day (deterministic)', () => {
        ...
        expect(next).toBe(s);
        expect(next.shop.adoptableCats.map((c) => c.id)).toEqual(s.shop.adoptableCats.map((c) => c.id));
    });
    it('regenerates exactly 5 cats on a new day', () => {
        ...
        expect(next.shop.adoptableCats.map((c) => c.id)).not.toEqual(s.shop.adoptableCats.map((c) => c.id));
    });
});
