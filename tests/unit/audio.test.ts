...
import { createAudio, type AudioBackend, type SfxName } from '../../src/audio/audio.js';
import type { Settings } from '../../src/game/types.js';

function fakeBackend() {
  const calls = { startMusic: 0, stopMusic: 0, oneShots: [] as SfxName[] };
...
describe('audio gating', () => {
  it('does not start music before the Start gesture', () => {
    const settings: Settings = { musicEnabled: true, sfxEnabled: true };
  ...
  it('respects musicEnabled', () => {
    const settings: Settings = { musicEnabled: false, sfxEnabled: true };
    const { backend, calls } = fakeBackend();
    const audio = createAudio(() => settings, backend);
    audio.markStarted();
    audio.startMusic();
  ...
  it('plays sfx only when enabled and after start', () => {
    const settings: Settings = { musicEnabled: true, sfxEnabled: false };
    const { backend, calls } = fakeBackend();
    const audio = createAudio(() => settings, backend);
    audio.markStarted();
    audio.playSfx('feed');
