...
describe('intro screen', () => {
  ...
  it('hides the intro and calls onStart when Start is pressed', () => {
    ...
    handle.show();
    (root.querySelector('.intro-start') as HTMLButtonElement).click();
    expect(started).toBe(true);
    expect((handle.root as HTMLElement).hidden).toBe(true);
  });

  it('binds the music and sfx toggles to settings and persists changes', () => {
    mount();
    (root.querySelector('.intro-settings') as HTMLButtonElement).click();
    const music = root.querySelector('.toggle-music') as HTMLInputElement;
    const sfx = root.querySelector('.toggle-sfx') as HTMLInputElement;
